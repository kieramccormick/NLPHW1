...
class Grammar:
    ...
    def _load_rules_from_file(self, grammar_file):
        ...
        with open(grammar_file,'r', encoding='utf-8') as f:
            for line in f:
                ...
                left_hand_side = parts[1]
                right_hand_side = parts[2].split()

                if left_hand_side not in self.rules:
                    self.rules[left_hand_side] = []

                self.rules[left_hand_side].append((weight, right_hand_side))
    ...
    def sample(self, derivation_tree, max_expansions, start_symbol):
        ...
        """
        def expand_symbol(symbol, expansions_count):
            if symbol not in self.rules:
                return symbol

            if expansions_count[0] >= max_expansions:
                return "..."

            expansions_count[0] += 1

            rules = self.rules[symbol]
            weights = [rule[0] for rule in rules]
            chosen_rule = random.choices(rules, weights=weights)[0]
            expansion = chosen_rule[1]

            expanded_parts = []
            for part in expansion:
                expanded_part = expand_symbol(part, expansions_count)
                expanded_parts.append(expanded_part)

            if derivation_tree:
                if len(expanded_parts) == 1 and expanded_parts[0] not in self.rules:
                    return f"({symbol} {expanded_parts[0]})"
                else:
                    return f"({symbol} {' ' .join(expanded_parts)})"
            else:
                return ' '.join(expanded_parts)

        expansions_count = [0]
        result = expand_symbol(start_symbol, expansions_count)
